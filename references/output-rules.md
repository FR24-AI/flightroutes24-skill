...
- **每次展示报价，必须来自本轮实际执行搜索脚本的输出**，不得使用对话历史中的旧搜索结果
- **严禁**：用对话历史中的报价数据直接回复用户；在未执行搜索脚本的情况下展示任何报价、价格、报价ID；声称"沿用上次结果"或"根据上次搜索"展示报价
- 旧报价数据（价格、报价ID）可能已过期失效，沿用会导致校验失败，必须每次重新搜索
