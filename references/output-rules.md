...
- **首次展示报价**：必须来自本轮实际执行的 `search` 脚本输出
- **用户从已展示列表选序号/航班号/报价ID**：必须执行 `select`，**禁止**为此重新 `search`
- **严禁**：用对话历史中的报价数据直接回复用户；在未执行 `search` 的情况下展示任何报价
- 改航司/日期/时段或校验 **304016** 后须 `refine` → 重新 `search`
